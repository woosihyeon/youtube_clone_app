...
class YoutubeRepository extends GetConnect {
  ...
  Future<YoutubeVideoResult?> loadVideos(String nextPageToken)async{
    String url = '/youtube/v3/search?part=snippet&channelId=UCbMGBIayK26L4VaFrs5jyBw&maxResults=10&order=date&type=video&videoDefinition=high&key=AIzaSyCfsSBWTspqXIWKdN20vTfEVv3R6k42xQE&pageToken=$nextPageToken';
  ...
  Future<Youtuber?> getYoutuberInfoById(String channelId)async{
    String url = '/youtube/v3/channels?part=statistics,snippet&key=AIzaSyCfsSBWTspqXIWKdN20vTfEVv3R6k42xQE&id=$channelId';
    final response = await get(url);
    if(response.status.hasError){
      return Future.error(response.statusText!);
    }else{
